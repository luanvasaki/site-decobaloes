      ...
      <div className="space-y-4 p-5 rounded-2xl bg-slate-50 border border-slate/8">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            {categories.length === 0 ? (
              <div className="w-full px-4 py-3 rounded-xl border border-amber-200 bg-amber-50 text-sm text-amber-700 flex items-center gap-2">
                <span>Nenhuma categoria cadastrada.</span>
                <a href="/admin/categorias/nova" className="font-bold underline hover:no-underline">
                  Criar agora →
                </a>
              </div>
            ) : (
          ...
          <div className="flex flex-wrap gap-2">
            {imageFiles.map((file, index) => (
              <div key={index} className="relative w-20 h-20 rounded-xl overflow-hidden border-2 border-dashed border-[#F9A8D4] group">
                {/* native img: Object URLs from file input are not supported by Next/Image */}
                {/* eslint-disable-next-line @next/next/no-img-element */}
